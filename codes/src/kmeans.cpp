...
namespace ANNS {
    ...
    float calc_distance(const char *vec_1, const char *vec_2, IdxType dim) {
        const float *x = reinterpret_cast<const float*>(vec_1);     // distance.cpp 中的计算方法
        const float *y = reinterpret_cast<const float*>(vec_2);
    ...
    void compute_vecs_l2sq(float *vecs_l2sq, float *data, const size_t num_points, const size_t dim)
    {
        ...
        {
            vecs_l2sq[n_iter] = cblas_snrm2((MKL_INT)dim, (data + (n_iter * dim)), 1);  // 计算 L2 范数
            vecs_l2sq[n_iter] *= vecs_l2sq[n_iter];     // L2 范数平方
        }
    }
    ...
    {
        if (k > num_centers)    // 最多 k 个最近邻聚类中心
        {
                                 ...
                                 float *pts_norms_squared)
    {
        ...
        float *pivs_norms_squared = new float[num_centers];     // 聚类中心的平方 L2 范数
        if (!is_norm_given_for_pts)
            pts_norms_squared = new float[num_points];      // 数据点的平凡 L2 范数
                      ...
                      std::vector<size_t> *closest_docs, uint32_t *&closest_center)
    {
        ...
        for (int64_t c = 0; c < (int64_t)num_centers; ++c)
        {
            ...
            for (size_t i = 0; i < dim; i++)
                cluster_sum[i] = 0.0;
            // 每个聚类中的所有数据点在每个维度上的和，所有点的均值作为新的聚类中心
            ...
            for (int64_t chunk = 0; chunk < (int64_t)nchunks; ++chunk)
                for (size_t d = chunk * CHUNK_SIZE; d < num_points && d < (chunk + 1) * CHUNK_SIZE; ++d)
                    residuals[chunk * BUF_PAD] +=
                        calc_distance(data + (d * dim), centers + (size_t)closest_center[d] * (size_t)dim, dim);
    ...
    void selecting_pivots(float *data, size_t num_points, size_t dim, float *pivot_data, size_t num_centers)
    {
        ...
        for (size_t j = 0; j < num_centers; j++)
        {
            ...
            std::memcpy(pivot_data + j * dim, data + tmp_pivot * dim, dim * sizeof(float)); // 存入选中的数据点
        }
    }
    ...
    {
        // 数据量大于 kmeanspp 的处理范围，直接用 kmeans 选取中心点
        if (num_points > 1 << 23)
        {
        ...
        for (int64_t i = 0; i < (int64_t)num_points; i++)
        {
            dist[i] = calc_distance(data + i * dim, data + init_id * dim, dim); // 平方距离
        }
        ...
        while (num_picked < num_centers)
        {
            ...
            for (int64_t i = 0; i < (int64_t)num_points; i++)
            {
                dist[i] = (std::min)(dist[i], calc_distance(data + i * dim, data + tmp_pivot * dim, dim));
            }
            num_picked++;
        }
        delete[] dist;
